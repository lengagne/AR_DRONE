...
//         cv::createTrackbar("ThresParam2", "in",&iThresParam2, 13, cvTackBarEvents);
	...
	bool inX, old_inX;
	bool inZ, old_inZ;
	float X,Y,Z,theta, oldX, oldZ, oldtheta, oldY;
...
//                 cout<<TheMarkers[i]<<endl;
	    ...
	    old_inZ = false;
			...
			{
				X =  TheMarkers[0].Tvec.ptr<float>(0)[0] ;	// side
				Y =  TheMarkers[0].Tvec.ptr<float>(0)[1] ;	// front
				Z =  TheMarkers[0].Tvec.ptr<float>(0)[2] ;	// height : ref 300
...
// 				yaw_ =   (  dtheta -0 )* 0.005;
				...
				pitch_ = ( (Y + 20*dY) ) * 0.002;
				
				gaz_ = ( 300- Z ) * 0.001;  // seems good
				roll_ = - ( 0 - (X + 20* dX)) * 0.002; 
				
				std::cout<<" X = "<< X << "\tY = "<< Y <<"\tZ = "<< Z <<"\t theta = "<< theta<<std::endl;
				std::cout<<" gaz_ = "<< gaz_ << "\t pitch_ = "<< pitch_ <<"\troll_ = "<< roll_ <<std::endl<<std::endl<<std::endl;
